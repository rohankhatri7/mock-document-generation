...
class DocumentGenerator:
        ...
        for field in self.spec.get('fields', []):
            ...
            for fn, _, _ in processed:
                ...
                if fn == 'City,State':
                    ...
                    parts = [
                        str(data.get('Street1', '')).strip(),  # Add str()
                        str(data.get('Street2', '')).strip(),  # Already has str()
                        str(data.get('City', '')).strip(),     # Add str()
                        str(data.get('State', '')).strip(),    # Add str()
                        str(data.get('Zip', '')).strip()       # Already has str()
                    ]
    ...
    def generate_document(self, data_row, output_path=None):
        if output_path is None:
            # Update default output directory
            output_dir = Path(__file__).parent.parent / 'output' / 'documents'
            output_dir.mkdir(parents=True, exist_ok=True)
            output_path = output_dir / f'{self.template_stem}.png'
...
class BatchGenerator:
    ...
    def generate_sequential(self, rows, out_dir, count=None):
        Path(out_dir).mkdir(parents=True, exist_ok=True)
        count = min(count, len(rows)) if count else len(rows)
        files = []
        
        for i in range(count):
            out = Path(out_dir) / f"{_make_output_stem(self.generator.template_stem, self.generator.quality)}{i + 1 if count > 1 else ''}.png"
            self.generator.generate(rows[i], output_path=out)
            files.append(str(out))
        
        return files
    
    def generate_parallel(self, tpl, spec, qual, rows, out_dir, count, workers):
        if not workers or workers < 2:
            return self.generate_sequential(rows, out_dir, count)
        ...
        with ProcessPoolExecutor(max_workers=workers) as pool:
            paths = list(pool.map(_render_worker, tasks))
...
class PDFConverter:
    ...
    def convert_single(self, image_paths):
        for pth in image_paths:
            pdf = self.pdf_dir / (Path(pth).stem + '.pdf')
            Image.open(pth).convert('RGB').save(pdf, 'PDF', resolution=300)
            print('Generated:', pdf)
    
    def convert_batch(self, image_paths, subtype):
        pdf = self.pdf_dir / f'{subtype}_batch.pdf'
...
def create_argument_parser():
    ...
    parser.add_argument('doc_subtype', nargs='?', help='Document subtype')
    parser.add_argument('--data', required=True, help='CSV data file path')
    parser.add_argument('--numdocs', type=int, default=1, help='Number of documents')
    parser.add_argument('--quality', choices=['clear', 'unclear'], default='unclear')
    parser.add_argument('--pdf', choices=['single', 'multi'], help='PDF output format')
...
class CommandLineProcessor:
    def __init__(self, args):
        self.args = args
        self.base = Path(__file__).parent.parent  # Points to root directory
    ...
    def run(self):
        try:
            ...
            subtype = self.args.doc_subtype
            
            if not subtype:
                raise ValueError('doc_subtype is required')
                
            # Updated paths to use root-level output directory
            tpl = self.base / 'output' / 'clean_templates' / f'{subtype}_clean.png'
            spec = self.base / 'output' / 'clean_templates' / f'{subtype}_spec.json'
            
            if not tpl.exists() or not spec.exists():
                raise FileNotFoundError(f'Template files not found for {subtype}. Run clean_template first.')
            
            numdocs = self.args.numdocs if self.args.numdocs else 1
            gen = DocumentGenerator(tpl, spec, None, 12, self.args.quality)
            batch_gen = BatchGenerator(gen)
            
            paths = batch_gen.generate_sequential(rows, self.out_dir, numdocs)
            ...
            if hasattr(self.args, 'pdf') and self.args.pdf:
                converter = PDFConverter(self.base / 'output' / 'pdfs')  # Updated path
                if self.args.pdf == 'single':
                    converter.convert_single(paths)
                else:
                    converter.convert_batch(paths, subtype)
...
def main():
    ...
    processor = CommandLineProcessor(args)
